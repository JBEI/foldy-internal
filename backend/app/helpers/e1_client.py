...
SequenceType = str
SequenceOrComplexType = SequenceType
...
class FoldyE1Client(FoldyESMClient):
    ...
    def __init__(self, model_name: str) -> None:
        ...
        if self.device.type == "cuda":
            ...
            try:
                ...
                logging.info(f"E1 client {self.model_name}: set multiprocessing start method to 'spawn'")
            except RuntimeError:
                logging.info(f"E1 client {self.model_name}: multiprocessing start method already set")
    ...
    def _lazy_load(self):
        if self._model is None:
            try:
                ...
                import multiprocessing
                logging.info(f"E1 {self.model_name} _lazy_load: current mp start method '{multiprocessing.get_start_method()}'")
                logging.info(f"Loading E1-{size} on {self.device}")
                self._preparer = E1BatchPreparer()
                self._model = E1ForMaskedLM.from_pretrained(
                    f"Profluent-Bio/E1-{size}m"
                ).to(self.device).eval()
                ...
                logging.warning(f"E1 unavailable (Python/version issue); skipping E1 features. Error: {e}")
                self._available = False
    ...
    def get_logits(
        ...
        try:
            ...
            L = len(sequence)
            assert residue_logits.shape[0] == L, f"Expected {L} residues, got {residue_logits.shape[0]}"
