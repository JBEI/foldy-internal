...
class TestCampaignWorldState:
    ...
    def test_initialization(self):
        ...
        world_state = CampaignWorldState(
            activity_df["DMS_score"],  # Converting to Series
            naturalness_df["wt_marginal"],  # Converting to Series
            embedding_df["embedding"],  # Converting to Series
        )

        # Verify initialization
        assert world_state.golden_activity_series.equals(activity_df["DMS_score"])
        assert world_state.naturalness_series.equals(naturalness_df["wt_marginal"])
        assert world_state.embedding_series.equals(embedding_df["embedding"])
...
class TestRunSingleSimulation:
    ...
    def test_run_single_simulation_first_round(self, mock_get_few_shot, mock_get_zero_shot):
        ...
        available_seq_ids = activity_df.index.tolist()
        activity_series = activity_df["DMS_score"]
        naturalness_series = naturalness_df["wt_marginal"]
        embedding_series = embedding_df["embedding"]
