...
def send_status_update_email(job: Job, status: str, extra_body: Optional[str]=None)-> None:
    ...
    if any(key not in job.meta for key in ['fold_id', 'fold_name', 'job_type', 'related_object_id']):
        logging.error(f"Job {job.id} has no fold_id, fold_name, job_type, or related_object_id in meta")
        return
