...
def run_evolvepro(evolve_id: int):
    ...
    with LoggingRecorder(invokation):
        ...
        if not evolve.embedding_files or not evolve.naturalness_files:
            raise ValueError(
                f"These days, evolve jobs must specify both embedding files (found {evolve.embedding_files}) and naturalness files (found {evolve.naturalness_files})"
            )
        if not evolve.few_shot_params:
            raise ValueError(
                f"These days, few shot params are required, got {evolve.few_shot_params}"
            )
        ...
        if evolve.num_mutants is None or evolve.num_mutants <= 0:
            raise ValueError(
                f"Evolve job must specify a positive number of mutants, got {evolve.num_mutants}"
            )
        ...
        # PROCESS INPUTS #######################################################
        activity_df, embedding_df, incomplete_naturalness_df = (
            process_and_validate_evolve_input_files(
            ...
            def get_relevant_measured_mutants(seq_id) -> str:
                return ", ".join(
                    sorted(
                        sum(
                            [
                                loci_to_measured_mutants.get(locus, [])[:5]
