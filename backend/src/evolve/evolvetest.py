...
def digivolve(wt_aa_seq,dataset,exp_activity_file_path,embeddings_dir, embeddings_paths,num_var,rounds_evo,model_type):
    ...
    for path in embeddings_paths:
        ...
        for rounds in range(rounds_evo):
            ...
            round_variants, top_ten_variants_found = evolve_simulation(wt_aa_seq,initial_round_activity,raw_embedding_df,exp_activity_df,ninety_percent_df,ninetyfive_percent_df,top_ten_df,model_type,num_var)
            for key in round_variants:
                if key not in path_results:
                    path_results[key] = []
                round = round_variants[key]
                path_results[key].append(round)
            print(f'This is {round_variants['top ten']} to top ten and {round_variants} is the output, {top_ten_variants_found} was found')
        for key in path_results:
            average = statistics.mean(path_results[key])
            std_err = sem(path_results[key])
            path_results[key] = [average,std_err]

            
        print(f'{path} embeddings gave {path_results}')
    results_path = os.path.join(embeddings_dir,dataset)
    path_results_df = pd.DataFrame.from_dict(path_results)
    display(path_results_df)
    try:
        os.mkdir(results_path)
    except OSError as error:
        print(error)
    comp_dir = os.path.join(embeddings_dir, dataset, f'{date.today()}{num_var}Variant_Model_Comparisons.xlsx')
    #path_results_df.to_excel(comp_dir, index=True)
    for sheet_name, sheet_data in path_results_df.items():
        # Convert the sheet data into a DataFrame
        df = pd.DataFrame(sheet_data)
        # Write the DataFrame to the Excel sheet
        df.to_excel(comp_dir, sheet_name=sheet_name, index=False)
    return(path_results)
