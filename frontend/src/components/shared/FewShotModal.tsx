...
interface FewShotModalProps {
    ...
    title?: string;
    templateFewShot?: Partial<FewShot> | null;
...
export const FewShotModal: React.FC<FewShotModalProps> = ({
    ...
    templateFewShot = null,
    defaultActivityFileSource = 'upload',
    fewShotCampaignRoundIdForActivityFile,
    fewShotRunIdForActivityFile
}) => {
    const [fewShotName, setFewShotName] = useState<string>(templateFewShot?.name || '');
    const [activityFile, setActivityFile] = useState<File | null>(null);
    const [activityFileSource, setActivityFileSource] = useState<'upload' | 'evolution' | 'campaign'>(defaultActivityFileSource);
    const [selectedFewShotForActivity, setSelectedFewShotForActivity] = useState<number | null>(fewShotRunIdForActivityFile || null);
    const [selectedCampaignRoundForActivity, setSelectedCampaignRoundForActivity] = useState<number | null>(fewShotCampaignRoundIdForActivityFile || null);
    const [mode, setMode] = useState<string>(templateFewShot?.mode || 'TorchMLPFewShotModel');
    const [numMutants, setNumMutants] = useState<number>(templateFewShot?.num_mutants || 24);
    const [selectedEmbeddingPaths, setSelectedEmbeddingPaths] = useState<string[]>(templateFewShot?.embedding_files ? templateFewShot.embedding_files.split(',') : []);
    const [selectedNaturalnessPaths, setSelectedNaturalnessPaths] = useState<string[]>(templateFewShot?.naturalness_files ? templateFewShot.naturalness_files.split(',') : []);
    const [finetuningModelCheckpoint, setFinetuningModelCheckpoint] = useState<string>(templateFewShot?.finetuning_model_checkpoint || 'facebook/esm2_t6_8M_UR50D');
    const [fewShotParams, setFewShotParams] = useState<string>(templateFewShot?.few_shot_params || '');
