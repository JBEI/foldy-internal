...
interface MutantSlateCardProps {
    ...
    onTemplateToggle: (seqId: string, isChecked: boolean) => Promise<void>;
                ...
                <div>
                    ...
                    <Table
                        ...
                        columns={[
                            ...
                            {
                                title: 'Mean Activity',
                                dataIndex: 'mean_activity',
                                key: 'mean_activity',
                                render: (activity: number | null) =>
                                    activity !== null ? activity.toFixed(4) : 'NA',
                                sorter: (a, b) => {
                                    const aVal = a.mean_activity ?? -Infinity;
                                    const bVal = b.mean_activity ?? -Infinity;
                            ...
                            {
                                title: (
                                    <Tooltip title="This mutant will be available as a template for later rounds of mutagenesis">
                                        <span>Template</span>
                                    </Tooltip>
                                ),
                                key: 'template',
                                render: (record: typeof tableData[0]) => {
                                    const isTemplate = (currentRound.promoted_templates || []).includes(record.seq_id);
                                    return (
                                        <Tooltip title="This mutant will be available as a template for later rounds of mutagenesis">
                                            <input
                                                type="checkbox"
                                                ...
                                                onChange={(e) => onTemplateToggle(record.seq_id, e.target.checked)}
                                                style={{ cursor: updatingTemplates ? 'not-allowed' : 'pointer' }}
                                            />
                                        </Tooltip>
                                    );
                                },
                    ...
                    {!hasActivityFile ? (
                        ...
                        <div style={{ marginTop: '24px' }}>
                            <Alert
                                message="Activity Results Loaded"
                                description={`Successfully loaded ${activityData?.length || 0} activity measurements.`}
                                type="success"
                                showIcon
